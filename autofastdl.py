...
class Logger:
	...
	def PrettyPrint(filename, status, tag = 'autofastdl'):
		...
		if 'docker' in config and config['docker']:
			...
			if status == "":
				text_status = ""
			text = tag_color + tag_text + filename_color + filename + text_status + Style.RESET_ALL
			print(text + '\n')
		else:
			columns = int(os.popen("stty size", 'r').read().split()[1])
			rows = math.ceil((len(filename) + len(status))/columns)
			text = tag_color + tag_text + filename_color + filename + '.'*(columns*rows - (len(tag_text) + len(filename) + len(status))) + color + status + Style.RESET_ALL
		...
		while True:
			...
			try:
				try:
					...
					Logger.PrettyPrint("Error sending a NOOP cmd ({0})".format(e), "", "error")
					Logger.PrettyPrint("Reconnecting...", "", "info")
					ftp = FTPHelper.GetConnection()

				if config['debug']:
					Logger.PrettyPrint("Job: {0}({1})".format(job[0].__name__, job[1]), "", "debug")
...
class AutoRemove:
	...
	def HandleFileUploaded(filepath, commonprefix):
		if AutoRemove.IsFileRemovedAfterUpload():
			with AutoRemove.files_removed_after_upload_lock:
				...
				Logger.PrettyPrint(os.path.relpath(filepath, commonprefix), "Deleted", "local")
				os.remove(filepath)
	...
	def CheckAllFiles(ftp, sourcedir, destdir):
		if not AutoRemove.IsAutoCleaned(AutoRemove.configFTP) and not AutoRemove.IsAutoCleaned(AutoRemove.configLocal):
			return
		...
		for dirpath, dirnames, filenames in os.walk(sourcedir):
			if not any(folder in dirpath for folder in config['ignore_folders']):
				...
				if AutoRemove.IsAutoCleaned(AutoRemove.configFTP):
					destdirectory = os.path.join(destdir, os.path.relpath(dirpath, os.path.join(sourcedir, "..")))
					jobs.put((AsyncFunc.CheckDirFTP, destdirectory))

				if AutoRemove.IsAutoCleaned(AutoRemove.configLocal):
					Logger.PrettyPrint("AutoCleanup in " + sourcedir, "Started", "local")
					filenames.sort()
					for filename in [f for f in filenames if f.endswith(config['extensions']) and f not in config['ignore_names']]:
						sourcefile = os.path.join(dirpath, filename)
						destfile = os.path.join(destdir, os.path.relpath(dirpath, os.path.join(sourcedir, "..")), filename + ".bz2")
						commonprefix = os.path.abspath(os.path.join(os.path.dirname(os.path.commonprefix(sourcefile)), ".."))

						if FTPHelper.file_exists(ftp, destfile):
							jobs.put((AsyncFunc.CheckFile, sourcefile, destfile, commonprefix, commonprefix_ftp))
						else:
							jobs.put((AsyncFunc.CheckFileLocal, sourcefile, commonprefix, None))
	...
	def CheckDirFTP(ftp, sourcedir, threaded = False):
		...
		for dirpath, dirnames, filenames in FTPHelper.walk(ftp, sourcedir):
			if not any(folder in dirpath for folder in config['ignore_folders']):
				filenames.sort()
				for filename in [f for f in filenames if f.endswith(ftp_extensions) and f not in ftp_ignore_names]:
					if threaded:
						jobs.put((AsyncFunc.CheckFileFTP, dirpath + '/' + filename, dirpath, None))
					else:
	...
	def CheckFile(ftp, sourcefile, destfile, sourcecommonprefix, destcommonprefix):
		if AutoRemove.configName in config:
			filedatetime = None
			if AutoRemove.configLocal in config[AutoRemove.configName] and AutoRemove.configPriority in config[AutoRemove.configName]:
				if AutoRemove.configLocal == config[AutoRemove.configName][AutoRemove.configPriority]:
					filedatetime = AutoRemove.GetTimestampFile(sourcefile)
				elif AutoRemove.configFTP == config[AutoRemove.configName][AutoRemove.configPriority]:
	...
	def CheckFileFTP(ftp, myfile, commonprefix, filedatetime = None):
		...
		if AutoRemove.IsOutdated(filedatetime, method):
			Logger.PrettyPrint(os.path.basename(myfile) + " (" + str(filedatetime) + ")", "Outdated", method)
	...
	def CheckFileLocal(myfile, commonprefix, filedatetime = None):
		...
		if AutoRemove.IsOutdated(filedatetime, method):
			...
			if AutoRemove.IsFileRemoved(method):
				os.remove(myfile)
				Logger.PrettyPrint(os.path.relpath(myfile, commonprefix), "Deleted", method)
				return False
			else:
				Logger.PrettyPrint(os.path.relpath(myfile, commonprefix), "Keeping", method)
		return True
	...
	def IsOutdated(mydatetime, method):
		if AutoRemove.configName in config and method in config[AutoRemove.configName]:
			...
			if 'days' in config[AutoRemove.configName][method]:
				checkTimeDelta += datetime.timedelta(days=config[AutoRemove.configName][method]['days'])
			if 'minutes' in config[AutoRemove.configName][method]:
				checkTimeDelta += datetime.timedelta(minutes=config[AutoRemove.configName][method]['minutes'])
			if 'seconds' in config[AutoRemove.configName][method]:
				checkTimeDelta += datetime.timedelta(seconds=config[AutoRemove.configName][method]['seconds'])				

			currenttime = datetime.datetime.now()

			if checkTimeDelta != datetime.timedelta(minutes=0) and mydatetime < currenttime - checkTimeDelta:
				return True
		return False

	@staticmethod
	def IsConfigGood(method):
		return AutoRemove.configName in config

	@staticmethod
	def IsFileRemoved(method):
		return AutoRemove.configName in config and method in config[AutoRemove.configName] and 'remove' in config[AutoRemove.configName][method] and config[AutoRemove.configName][method]['remove']

	@staticmethod
	def IsFileRemovedAfterUpload():
		return AutoRemove.configName in config and 'after_upload' in config[AutoRemove.configName] and config[AutoRemove.configName]['after_upload']

	@staticmethod
	def IsStartupClean(method):
		return AutoRemove.configName in config and method in config[AutoRemove.configName] and 'startup_clean' in config[AutoRemove.configName][method] and config[AutoRemove.configName][method]['startup_clean']

	@staticmethod
	def IsAutoCleaned(method):
		return AutoRemove.configName in config and method in config[AutoRemove.configName] and 'autoclean' in config[AutoRemove.configName][method] and config[AutoRemove.configName][method]['autoclean']

	@staticmethod
	def GetTimezone(method, name):
		return AutoRemove.configName in config and method in config[AutoRemove.configName] and name in config[AutoRemove.configName][method] and config[AutoRemove.configName][method][name]

class AsyncFunc:

	@staticmethod
	def CheckFile(ftp, item):
		sourcefile, destfile, sourcecommonprefix, destcommonprefix = item

		AutoRemove.CheckFile(ftp, sourcefile, destfile, sourcecommonprefix, destcommonprefix)

	@staticmethod
	def CheckFileFTP(ftp, item):
		myfile, commonprefix, filedatetime = item

		AutoRemove.CheckFileFTP(ftp, myfile, commonprefix, filedatetime)

	@staticmethod
	def CheckFileLocal(ftp, item):
		myfile, commonprefix, filedatetime = item
	...
	def CheckFileAdd(ftp, item):
		...
		if AutoRemove.CheckFileLocal(sourcefile, commonprefix):
			Logger.PrettyPrint(os.path.relpath(sourcefile, commonprefix), "Added", "remote")
	...
	def Compress(ftp, item):
		...
		try:
			...
			Logger.PrettyPrint(os.path.relpath(sourcefile, commonprefix), "Done", "remote")
			AutoRemove.HandleFileUploaded(sourcefile, commonprefix)
		except:
			print("Unexpected error:", sys.exc_info())
			Logger.PrettyPrint(os.path.relpath(sourcefile, commonprefix), "Failed (Skipping)", "remote")
	...
	def Delete(ftp, item):
		...
		try:
			...
			Logger.PrettyPrint(os.path.relpath(item, commonprefix_ftp), "Deleted", "remote")
	...
	def Move(ftp, item):
		...
		Logger.PrettyPrint("{0} -> {1}".format(os.path.relpath(sourcepath, commonprefix_ftp), os.path.relpath(destpath, commonprefix_ftp)), "Moved", "remote")
...
class EventHandler(pyinotify.ProcessEvent):
	...
	def process_IN_CLOSE_WRITE(self, event):
		...
		if not event.pathname.endswith(config['extensions']) or os.path.basename(event.pathname) in config['ignore_names'] or any(folder in event.pathname for folder in config['ignore_folders']):
			return
	...
	def process_IN_MOVED_TO(self, event):
		...
		if event.dir:
			...
			if event.src_pathname.endswith(config['extensions']) or os.path.basename(event.pathname) in config['ignore_names'] or any(folder in event.pathname for folder in config['ignore_folders']):
				return

			if not event.src_pathname.endswith(config['extensions']) and event.pathname.endswith(config['extensions']):
				# Renamed invalid_ext file to valid one -> compress
				jobs.put((AsyncFunc.Compress, event.pathname, destpath + ".bz2"))
				return

			elif event.src_pathname.endswith(config['extensions']) and not event.pathname.endswith(config['extensions']):
...
class DirectoryHandler:
	...
	def Do(self, ftp): # Normal mode
		for dirpath, dirnames, filenames in os.walk(self.SourceDirectory):
			if not any(folder in dirpath for folder in config['ignore_folders']):
				...
				destdirectory = os.path.join(self.DestinationDirectory, os.path.relpath(dirpath, os.path.join(self.SourceDirectory, "..")))
				jobs.put((AsyncFunc.CheckDirFTP, destdirectory))
	...
	def Checkfile(self, ftp, dirpath, filename):
		...
		if FTPHelper.file_exists(ftp, destfile):
			Logger.PrettyPrint(os.path.relpath(sourcefile, commonprefix), "Exists", "local")
			jobs.put((AsyncFunc.CheckFile, sourcefile, destfile, commonprefix, commonprefix_ftp))
		else:
